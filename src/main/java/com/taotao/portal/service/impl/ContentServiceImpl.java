...
public class ContentServiceImpl implements ContentService {
	...
	public String getContentList() {
		...
		String jsonString = HttpClientUtils.doGet(REST_BASE_URL + REST_INDEX_AD_URL);
		// 需要把字符串转换成java对象taotaoResult对象
		TaotaoResult resultFromJson = TaotaoResult.formatToList(jsonString, TbContent.class);
		// 从taotaoResult对象中取data属性，得到内容列表
		List<TbContent> contentList = (List<TbContent>) resultFromJson.getData();
		...
		String contentListResultJson = JsonUtils.objectToJson(jsonListMapData);
		// 返回一个json字符串。
